...
int main()
{
    ...
    Model donut(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 0.0f), ("keinpfad"), ("dateien/wege/donut_e.obj"));
    ...
    Model model8(glm::vec3(33.0f, 0.0f, 0.0f), glm::vec3(60.0f, 0.0f, 0.0f), ("dateien/wege/animation/weg8anim.txt"), ("dateien/wege/weg8.obj"));
    Model model9(glm::vec3(-33.0f, 0.0f, 0.0f), glm::vec3(60.0f, 0.0f, 0.0f), ("dateien/wege/animation/weg9anim.txt"), ("dateien/wege/weg9.obj"));
    ...
    vector<Model> modelvec{ model1, model2, model4, model3, model5, model8, model9 /*model5 */ };
    ...
    while (!glfwWindowShouldClose(window))
    {
       ...
       // cout << current_upV[0]<<" "<<current_upV[1]<<" "<<current_upV[2] << "\n";
        ...
        donutShader.use();
        donutShader.setVec3("light.direction", glm::vec3(autoAPos[3]) - current_cPos);
        donutShader.setVec3("light.ambient", 1.0f, 1.0f, 1.0f);
        donutShader.setVec3("light.diffuse", 0.1f, 0.1f, 0.1f);
        donutShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);
        donutShader.setMat4("projection", projection);
        donutShader.setMat4("view", view);


        donutShader.setFloat("material.shininess", 16.0f);

        donutShader.setVec3("viewPos", camera.Position);
        modelShader.use();
        for (int i = 0; i < 100; i++) {
            if (donuts[i].spur < 4) {
                if (donuts[i].startframe == currentAnimC) {
                    ...
                    cout << "Donuts gesammelt: " << score << "   Donuts verloren: " << insgesamtdonuts - score << "    Gewinnrate: "<<(float)score/insgesamtdonuts<<"\n";
                    ...
                    if (currentAnimC > donuts[i].startframe) {
                        ...
                        glm::vec3 modelrot = globalrot +currentwege[1].rotation;
                      /*  emodel = glm::rotate(emodel, glm::radians(modelrot.x), glm::vec3(1.0f, 0.0f, 0.0f));

                        emodel = glm::rotate(emodel, glm::radians(modelrot.y), glm::vec3(0.0f, 1.0f, 0.0f));
                        emodel = glm::rotate(emodel, glm::radians(modelrot.z), glm::vec3(0.0f, 0.0f, 1.0f));
                    */
                    }
                    else {
                    ...
                    */
                    ...
                    emodel = glm::rotate(emodel, glm::radians(donuts[i].rotation.x), glm::vec3(1.0f, 0.0f, 0.0f));

                    emodel = glm::rotate(emodel, glm::radians(donuts[i].rotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
                    emodel = glm::rotate(emodel, glm::radians(donuts[i].rotation.z), glm::vec3(0.0f, 0.0f, 1.0f));
                    ...
                    modelShader.setVec3("light.ambient", 0.7f, 0.2f, 0.5f);
                    modelShader.setVec3("light.diffuse", 0.4f, 0.4f, 0.4f);
                    modelShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);
        ...
        if (rand() % 60 == 1) {
            ...
            glm::vec3 abstand = glm::vec3(glm::normalize(currentwege[2].uMatrices[currentAnimC][3] - currentwege[2].aMatrices[currentAnimC][3]) * glm::vec4(20.0f, 20.0f, 20.0f, 1.0f));
...
vector<modelData> getwürfel(int num,glm::vec4 wegpos, glm::vec3 rightvec, glm::vec3 upvec, glm::vec3 frontvec) {
    ...
    glm::vec3 würfelfarbe(0.30, 0.10, 0.2); //alle würfel werden pastell mit tendenz zu lila

    for (int c = 0; c < num; c++) {
        glm::vec3 color((float)(rand()) / (float)(RAND_MAX) + würfelfarbe.r, (float)(rand()) / (float)(RAND_MAX)+würfelfarbe.g, (float)(rand()) / (float)(RAND_MAX)+würfelfarbe.b);
        glm::vec3 pos_e(wegpos);
        pos_e += rightvec * glm::vec3(8.0f);
        pos_e += upvec * glm::vec3( (float)( rand() % 20 - 10));
        pos_e += frontvec * glm::vec3((float)(rand() % 40-30));
